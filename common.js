/**
 * Commonly used routines throughout ECMA-402 package.  Also referrred to in the standard as "Abstract Operations"
 */
define(
	[   "List",
	    "Record",
	...
	function(List,Record,availableLocales_json, aliases_json, localeAliases_json, parentLocales_json) {
		var common = {
			...
			IsStructurallyValidLanguageTag : function(locale) {
				...
				var identifier = common._toLowerCaseIdentifier(locale);
			...
			DefaultLocale : function() {
				...
				if(common.IsStructurallyValidLanguageTag(navigator.language)){
					result = common.BestFitAvailableLocale(common.availableLocalesList, common.CanonicalizeLanguageTag(navigator.language));
				}
				if(!result&&common.IsStructurallyValidLanguageTag(navigator.userLanguage)){
					result = common.BestFitAvailableLocale(availableLocalesList,
						common.CanonicalizeLanguageTag(navigator.userLanguage));
				}
				if(!result){
					result = "root";
				}
			...
			CanonicalizeLocaleList : function(locales) {
				...
				for(Pk in O){
					...
					if(!common.IsStructurallyValidLanguageTag(tag)){
						throw new RangeError(tag+" is not a structurally valid language tag.");
					}
					tag = common.CanonicalizeLanguageTag(tag);
			...
			LookupMatcher : function(availableLocales, requestedLocales) {
				...
				while (i<len&&availableLocale===undefined){
					...
					noExtensionsLocale = locale.replace(common.unicodeLocaleExtensions, "");
				...
				if(availableLocale){
					...
					if(locale!=noExtensionsLocale){
						result.set("extension", locale.match(common.unicodeLocaleExtensions)[0]);
						result.set("extensionIndex", locale.search(common.unicodeLocaleExtensions));
					}
				}else{
					result.set("locale", common.DefaultLocale());
				}
			...
			BestFitMatcher : function(availableLocales, requestedLocales) {
				...
				while (i<len&&availableLocale===undefined){
					locale = requestedLocales[i];
					noExtensionsLocale = locale.replace(common.unicodeLocaleExtensions, "");
					availableLocale = common.BestFitAvailableLocale(availableLocales, noExtensionsLocale);
					i++;
				}
				var result = new Record();
				if(availableLocale){
			...
			ResolveLocale : function(availableLocales, requestedLocales, options, relevantExtensionKeys, localeData) {
				...
				var r = matcher=="lookup" ? common.LookupMatcher(availableLocales, requestedLocales) : common.BestFitMatcher(
			...
			LookupSupportedLocales : function(availableLocales, requestedLocales) {
				...
				while (k<len){
					...
					var noExtensionsLocale = locale.replace(common.unicodeLocaleExtensions, "");
			...
			BestFitSupportedLocales : function(availableLocales, requestedLocales) {
				...
				while (k<len){
					var locale = requestedLocales[k];
					var noExtensionsLocale = locale.replace(common.unicodeLocaleExtensions, "");
					var availableLocale = common.BestFitAvailableLocale(availableLocales, noExtensionsLocale);
					if(availableLocale!==undefined){
						subset.push(locale);
					}
					k++;
		...
		common.availableLocalesList = common.CanonicalizeLocaleList(availableLocales_json.availableLocales);
		return common;
	});