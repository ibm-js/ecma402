		...
		var testIntl = {
			...
			getLocaleSupportInfo : function(Constructor) {
				...
				for(i = 0; i<allTags.length; i++){
					...
					var result = new Constructor([ request ], {
			...
			isCanonicalizedStructurallyValidLanguageTag : function(locale) {
				...
				var alpha = "[a-zA-Z]", digit = "[0-9]", alphanum = "("+alpha+"|"+digit+")", regular = "(art-lojban|cel-gaulish|no-bok|no-nyn|zh-guoyu|zh-hakka|zh-min|zh-min-nan|zh-xiang)", irregular = "(en-GB-oed|i-ami|i-bnn|i-default|i-enochian|i-hak|i-klingon|i-lux|i-mingo|i-navajo|i-pwn|i-tao|i-tay|i-tsu|sgn-BE-FR|sgn-BE-NL|sgn-CH-DE)", grandfathered = "("
					+irregular+"|"+regular+")", privateuse = "(x(-[a-z0-9]{1,8})+)", singleton = "("+digit
					+"|[A-WY-Za-wy-z])", extension = "("+singleton+"(-"+alphanum+"{2,8})+)", variant = "("+alphanum
					+"{5,8}|("+digit+alphanum+"{3}))", region = "("+alpha+"{2}|"+digit+"{3})", script = "("+alpha
					+"{4})", extlang = "("+alpha+"{3}(-"+alpha+"{3}){0,2})", language = "("+alpha+"{2,3}(-"+extlang
					+")?|"+alpha+"{4}|"+alpha+"{5,8})", langtag = language+"(-"+script+")?(-"+region+")?(-"+variant
					+")*(-"+extension+")*(-"+privateuse+")?", languageTag = "^("+langtag+"|"+privateuse+"|"
					+grandfathered+")$", languageTagRE = new RegExp(languageTag, "i");
				var duplicateSingleton = "-"+singleton+"-(.*-)?\\1(?!"+alphanum+")", duplicateSingletonRE = new RegExp(
					duplicateSingleton, "i"), duplicateVariant = "("+alphanum+"{2,8}-)+"+variant+"-("+alphanum
			...
			testOption : function(Constructor, property, type, values, fallback, testOptions) {
				...
				testValues.forEach(function(value) {
					...
					if(noReturn){
						assert(!obj.resolvedOptions().hasOwnProperty(property), "Option property "+property
							+" is returned, but shouldn't be.");
					}else{
						...
						if(isILD){
							...
							assert(actual===expected||(isOptional&&actual===undefined), "Option value "+value
								+" for property "+property+" was not accepted; got "+actual+" instead.");
						}
					}
				});
				...
				if(type==="string"){
					...
					invalidValues.forEach(function(value) {
						...
						assert.isDefined(error, "Invalid option value "+value+" for property "+property
							+" was not rejected.");
						assert.strictEqual(error.name, "RangeError", "Invalid option value "+value+" for property "
							+property+" was rejected with wrong error "+error.name+".");
					});
				}
			...
			testProperty : function(obj, property, valid) {
				...
				assert(desc.writable, "Property "+property+" must be writable.");
				assert(desc.enumerable, "Property "+property+" must be enumerable.");
				assert(desc.configurable, "Property "+property+" must be configurable.");
			...
			mayHaveProperty : function(obj, property, valid) {
				if(obj.hasOwnProperty(property)){
					this.testProperty(obj, property, valid);
				}
			},
			/**
